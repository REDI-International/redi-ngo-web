...
export function BlockEditorPanel({
  ...
  block: import("@/lib/blocks/types").PageBlock;
  onUpdate: (patch: Partial<import("@/lib/blocks/types").PageBlock>) => void;
  onDelete: () => void;
  onClose: () => void;
}) {
  ...
  return (
    <aside className="fixed inset-y-0 right-0 z-[160] flex w-full max-w-md flex-col border-l border-black/10 bg-white shadow-2xl">
      <div className="flex items-center justify-between border-b border-black/10 px-5 py-4">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-[#86868b]">Edit section</p>
          <h2 className="font-heading text-lg font-bold text-text">{block.title ?? "Untitled"}</h2>
        </div>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <ImageUploadField value={block.imageUrl} onChange={(url) => onUpdate({ imageUrl: url })} />
        ...
        <label className="block">
          ...
          <select
            value={block.width}
            onChange={(e) => onUpdate({ width: e.target.value as import("@/lib/blocks/types").BlockWidth })}
            className="mt-1 w-full rounded-lg border border-black/10 px-3 py-2 text-sm"
          >
